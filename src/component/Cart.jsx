...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    const updatedCart = storedCart.map((item) => ({
      ...item,
      quantity: 1, // Ensure every item quantity is set to 1
    }));
    setCartItems(updatedCart);
  }, []);
    ...
    <div className="container">
      ...
      <div className="row my-4">
              ...
              {cartItems.map((item, index) => (
                <li key={index} className="list-group-item mb-4 border-1">
                  <div className="row">
                    ...
                    <div className="col-lg-6 py-1">
                      <h5>{item.title}</h5>
                      <p>{truncateDescription(item.description, 150)}</p> {/* Limit description length here */}
                      <p>${item.price}</p>
                    </div>
                    <div className="col-lg-3 d-flex flex-column">
                      <div className="Quantity py-1 d-flex align-items-center">
        ...
        <div className="col-lg-3">
          <div className="border p-3 rounded">
            <h4>Cart Summary</h4>
            <p>Total Items: {calculateTotalItems()}</p> {/* Display total items count */}
            <h5>Total Price: ${calculateTotalPrice()}</h5>
            <button className="btn btn-outline-warning ssssssss btn-lg my-5 px-5 rounded-5 mt-4" onClick={toggleModal}>
              Checkout
            </button>
